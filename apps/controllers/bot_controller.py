...
class MessageV2(BaseController, Resource):
    ...
    def post(self):
        ...
        if helper.is_booking_request(message) or ctx.get("active"):
            ...
            # 4.c: Spa — từ câu nói, từ last_spa_focus, hoặc gợi ý theo dịch vụ
            if not ctx.get("spa_name"):
                ...
                if spa_from_msg:
                    ...
                    if last_focus:
                        ...
                        if len(spas_for_service) == 0:
                            return helper.finalize_reply("Dịch vụ này hiện chưa có spa nào trong hệ thống. Bạn muốn chọn dịch vụ khác không?", conversation_key, history)
                        elif len(spas_for_service) == 1:
                            ctx["spa_name"] = spas_for_service[0]["name"]
                        else:
                            slot_label = ctx["slot"]["label"] if ctx.get("slot") else None
                            return helper.reply_choose_spa_for_service(ctx["service_name"], spas_for_service, conversation_key, history, slot_label=slot_label)
                    else:
                        helper.set_booking_context(user_id, {**ctx, "active": True})
                        return helper.finalize_reply("Bạn muốn đặt **dịch vụ** nào và tại **spa** nào ạ? (Bạn có thể trả lời: 'tên dịch vụ + tên spa')", conversation_key, history)

            # 4.d: Nếu vẫn CHƯA có slot → gợi ý slot
            if not ctx.get("slot"):
